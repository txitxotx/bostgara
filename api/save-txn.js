...
// Fixes: lib/github (retry, timeout, SHA race), auth backend, no expone err interno
...
export default async function handler(req, res) {
  ...
  if (!GITHUB_TOKEN || !GITHUB_REPO) {
    return res.status(500).json({ error: 'Faltan variables de entorno: GITHUB_TOKEN y GITHUB_REPO' });
  }
  ...
  try {
    ...
    await putFile(
      ...
      'transacciones.json',
      content,
      `Actualizar transacciones.json [${today}]`,
      GITHUB_BRANCH,
    );
    ...
    return res.status(500).json({ error: 'Error al guardar transacciones.json' });
  }
}
